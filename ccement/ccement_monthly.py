...
referer_pattern = 'https://price.ccement.com/Price_list-{page}-s{begin}-e{end}-p{province}-c{city}-k0-b0.html'
...
def get_price_from(page, province_code, city_code, begin_date, end_date):
    # 根据不同的页数, 省市的code构造请求
    referer = referer_pattern.format(page=page, province=province_code, city=city_code, begin=begin_date, end=end_date)
    headers = HEADERS
    headers.update({'referer': referer})
    query = QUERY
    query.update({'page': str(page), "provinceCode": province_code, "cityCode": city_code, "startTime": begin_date,
                  "endTime": end_date})
    ...
    try:
        ...
        output_df = pd.DataFrame(tuples,
                                 columns=['页数', '品牌', '水泥品种', '省份', '城市', '区域', '生产厂家', '日期', '散装价', '袋装价', '参考价'])
...
if __name__ == '__main__':
    ...
    for i in range(12):
        begin_date = '2019' + (str(i + 1) if i > 9 else '0' + str(i + 1)) + '01'
        end_date = '2019' + (str(i + 1) if i > 9 else '0' + str(i + 1)) + '28'
        for pc in pcs:
            province = pc.name
            province_code = pc.id
            ...
            for city_code, city in pc.cities.items():
                ...
                while 1:
                    print(
                        'page=' + str(page) + ', province_code=' + str(province_code) + ', city_code=' + str(city_code))
                    res, cur_output = get_price_from(page, province_code, city_code, begin_date, end_date)
                    if not res or page > 335:
                        break
                    # print(cur_output)
                    print(output)
                    ...
                    page += 1
                    time.sleep(10)
                output.to_csv('output_' + province + '_' + city + '_' + begin_date + '_' + end_date + '.txt', sep='\t',
                              index=False)
